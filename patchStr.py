...
decrypted_str = ["n001","(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;IZ)V",
"n002",
...
while addr < rodata_end:
    str = get_string(addr) #去获取到字符串
    str_len = len(str) + 1
    if all(c in string.hexdigits for c in str) and str_len > 8:
 
        i = 0
        tmp_addr = addr
        decrypted_strlen = len(decrypted_str[index]) #获取解密后的字符串
 
        while i < str_len:
            if i >= decrypted_strlen:
                patch_byte(tmp_addr, 0x00)
            else:
                patch_byte(tmp_addr, ord(decrypted_str[index][i])) #每次patch 一个字节
            i = i + 1 #地址对应的往上加
            tmp_addr = tmp_addr + 1
        index = index + 1
